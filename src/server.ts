...
export default async (): Promise<Express> => {
  ...
  app.use(cors())
  app.use((req, _, next) => {
    // make sure we always have a file object on req even if this is not a multipart body this is so that
    // the attachment route can handle both JSON and multipart bodies
    req.files = []
    next()
  })
